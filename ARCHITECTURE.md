...
**Wire format (v4).** Custom little-endian binary layout. A one-time header carries body names + µ, DP853 telemetry, and the timestep count. Each snapshot carries its UTC display timestamp and continuous SI milliseconds from J2000. The continuous key preserves the instant across and inside leap seconds, and drives reference queries and Hermite intervals. Time quantization is at most half a millisecond. The two int64 values cost 16 bytes per snapshot before compression, independent of body count. Positions ride as a per-body `float64` reference (timestep 0) plus per-step `float32` deltas; the client reconstructs absolute positions by prefix sum, and accumulated drift stays sub-kilometre over a chunk in the measured scenarios. Velocities are `float32` temporal-deltas (row 0 absolute, the rest per-step deltas), reconstructed the same way with ~0.02 m/s error in the measured scenarios. Both `float32` planes (position deltas and velocities) are laid out structure-of-arrays *and* byte-plane shuffled: each value's 4 bytes are split into 4 contiguous runs, so zstd sees long stretches of stable high-order bytes and compresses them hard. The whole body is zstd-compressed (level 3) with a 4-byte little-endian uncompressed-size prefix. The Web Worker un-shuffles positions and velocities into `Float64Array` buffers, reads both timestamp planes into `Float64Array` buffers, and transfers the buffers to the main thread without cloning. Redux then bulk-copies them into its long-lived rolling buffer.
...
### Global simulation admission

A 50-slot reservation covers both pending initialization and live sessions. Failed initialization, explicit removal, and idle eviction return the reservation exactly once. First chunks and speculative next chunks share a compute budget of `max(2, availableProcessors / 2)`. A first-chunk request that cannot reserve compute capacity returns 503 without advancing its session; speculative work is skipped when capacity is occupied. No unbounded computation backlog is created. Reservations remain held until computation ends even if its result is cancelled. These are concurrency bounds, not a measured heap guarantee; deployment sizing still requires representative load tests.
...
**Conserving system mass and motion.** Orekit's DE-440 outer-planet GM and position describe entire planetary systems. Before integration, each selected massive moon is split out: its GM is subtracted from the parent, and the residual parent's position and velocity are adjusted so the combined GM, barycenter, and momentum equal the original system record. This also handles partial moon selections. Omitted moons and selected test particles retain their physical mass in the collapsed parent; their motion is not resolved in that parent model. The residual is therefore an approximation to the physical planet center, especially for partially resolved systems. Rounded moon GM values and different ephemeris fits limit center accuracy. See [JPL system parameters](https://ssd.jpl.nasa.gov/astro_par.html).

**Classification.** Eight bodies are massive (feel and exert mutual gravity): Earth's Moon, the four Galileans (allowing mutual perturbations), Titan, Triton, and Charon. Charon is massive despite being ~10× lighter than Triton because it's ~12% of Pluto's mass, the largest moon-to-planet ratio in the solar system, and the Pluto-Charon barycenter only dances if both bodies are massive. The other 14 are test particles (their backreaction is omitted by the simplified model), sorted into the test suffix of the `[massive | test]` state layout by the same `SimulationFactory` partition that handles minor bodies.
...
### Reference overlay: matching times and planetary systems

The overlay compares the simulated path with Orekit's DE-440 reference for planets and Pluto. Numeric distance and angle use the **last saved snapshot** and a reference sample at its continuous millisecond time key. The scene is smoothed between snapshots, but those interpolated positions are never reported as new numeric measurements. The difference includes numerical integration, wire quantization, initial-state precision, and simplified physics; it is not a pure integrator error estimate. Energy conservation remains a separate diagnostic within the simulated model.

**Bounded exact sampling.** A sessionless POST to `/api/simulation/ground-truth` carries at most 2,000 continuous millisecond keys from J2000. Bounding the time window instead of coarsening the cadence avoids sparse Hermite interpolation appearing as thousands of kilometres of numerical error. The backend samples each instant directly, including irregular rounded adaptive-step timestamps and instants inside UTC leap seconds. The legacy cadence-based GET remains available. A static body allowlist prevents reference requests from causing Horizons network traffic.

The client keeps a bounded window behind and ahead of playback. It refetches in either direction, retries failed initial loads with a three-second playback throttle, and rejects superseded responses. Coverage comes from the returned samples. Numeric values require an exact matching sample; geometry is hidden outside coverage, and reference trails are clipped to that coverage. Compared with the former 400-anchor window, responses can be about five times larger, in exchange for removing interpolation error from the readout.

**Like-for-like system comparison.** DE-440 records for Mars through Pluto describe planetary systems. When selected massive moons are resolved, the reference compares the simulated GM-weighted center of mass of the residual parent and those moons with the ephemeris system barycenter. Earth and its Moon already have separate records. The UI labels these system comparisons explicitly.

### Preset clips: canonical scenarios as static edge assets

Every canonical scenario (the builders' untouched default plus the four catalog quick-selects) is precomputed once and committed as a static binary bundle (`clip-<presetId>-v4-science2.bin`) that Cloudflare Pages serves from the edge: the captured initialize response plus the first several wire-format chunks, played back through the same decode path as a live stream but with no session, so a preset run costs the backend nothing. Both builders intercept Run with an **exact** match (same epoch, frame, integrator, step, fidelity, and a set-equal body selection); anything else runs live, because a false positive would serve the wrong physics while a false negative only costs a live session. A single frontend registry drives the generator's preset table, the loader, and a staleness-guard test that fails CI whenever a committed asset no longer matches the registry, the shared preset constants, or the wire version.
