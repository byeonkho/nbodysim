...
**Per-parent render scale.** Heliocentric log compression (the Stylized preset's `A=60`, `r_ref=1 AU`) crushes parent-relative moon distances to a tiny fraction of a world unit, so every moon would clamp to the min-separation floor and pile on top of its parent. The scale pipeline now consults a `MOON_LOG_SCALE` map keyed by parent planet; when a body's parent isn't the Sun, `worldDistanceFromParent` applies that parent's own log curve (`A=5`, `r_ref` = the parent's innermost-moon real distance) instead of the heliocentric one, so each planet system spreads into its own readable cluster rather than collapsing to a ring. The parent name is resolved once per scene component (memoized, with a cached body index) so the per-frame render path stays allocation-free.
