...
**DP853 (adaptive)** runs Mode C time-gap thinning instead — emit when sim-time since the last emission crosses `chunk_duration / (N-1)`, where N is the bucket's target snapshot count. Drift-free targeting (advance the schedule by exactly `gap` each tick, not by where the actual emission landed) keeps the per-chunk count within ≪1% of N. Cross-chunk continuity via a per-session `nextEmitTarget` AbsoluteDate. Bucket→N table: Low/3000, Med-Low/5000, Medium/7500, Med-High/10000, High/15000. The DP853 tier is heavier on opt-in (up to 6 MB compressed at the highest bucket vs the 2 MB default-tier ceiling) — discoverable accuracy for users who opt in, default flows land on the fixed-step integrators.
...
**Wire compactness.** Positions and velocities are float32 on the wire (timestamp stays int64, µ stays float64). ~7-decimal-digit precision is fine for visualisation; halves raw per-timestep bytes. Combined with Mode C, DP853 default chunks dropped from ~16 MB compressed (the old "emit every accepted substep + throw at MAX_SNAPSHOTS_PER_CHUNK" model) to ~1 MB compressed.

Full rationale + math + rollout: Hermite in `docs/superpowers/specs/2026-05-15-hermite-keyframe-interpolation-design.md`; DP853 emission model + float32 in `docs/superpowers/specs/2026-05-18-dp853-emission-model-design.md`.
