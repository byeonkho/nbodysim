...
### Cubic Hermite interpolation + backend keyframe thinning

The backend emits every Kth integration step rather than every step (controlled by a per-session `keyframeIntervalSec` request param, resolved to `K = max(1, round(intervalSec / stepDt))` at the controller boundary, validated to `1..MAX_KEYFRAMES_PER_KEPT=100`). Cross-chunk continuity is preserved by a monotonic `globalStepCount` cursor inside `Simulation`, so chunk N+1's first kept frame lands exactly K steps after chunk N's last. The frontend uses cubic Hermite interpolation between kept keyframes — analytic-tangent form using the integrator's exact velocities (already on the wire), no estimation. Per-keyframe timestamps are always shipped, so non-uniform spacing is handled by the read path with no protocol change. The user picks a "Playback quality" preset in SimSetupDrawer (5 buckets, per-integrator default — Euler→K=1, RK4→K=4, DP853→K=8), or a custom 1–100 override. **Trade-offs:** with K>1 the visual "step" between samples is interpolated rather than computed, so high K on Euler/RK4 magnifies their pre-existing integration error visually. Per-integrator defaults guard the default flow; tooltip telegraphs the tradeoff to power users. K=1 path is a single integer compare per step — zero cost vs. pre-thinning. Hermite at integer keyframe indices short-circuits to a direct typed-array read, so existing integer-index callers (Trail tail loop) keep the no-allocation fast path. Wire format unchanged. Full rationale + math + rollout in `docs/superpowers/specs/2026-05-15-hermite-keyframe-interpolation-design.md`. Unblocks the planned DP853 native-substep-cadence work ([#68](https://github.com/byeonkho/spacesim/issues/68)).
