    ...
    public static State parseFirstRecord(String responseText) {
        ...
        if (soe < 0 || eoe < 0 || eoe < soe) {
            throw new IllegalArgumentException(
                "Horizons response missing $$SOE / $$EOE markers");
        }
        ...
        if (!pos.find()) {
            throw new IllegalArgumentException("No X/Y/Z line in Horizons block");
        }
        if (!vel.find()) {
            throw new IllegalArgumentException("No VX/VY/VZ line in Horizons block");
        }
